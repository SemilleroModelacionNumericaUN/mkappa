...
% paso e iteración correspondientes
function [SFZ,SMX,EA,EIX,EIY,GJ,EPZZ,STZZ,ESTA]=SUMF(CAT,MAT,NELE,ECA,ELE,k,yn,IPAS,EPZZ,STZZ,ESTA,E)
%
...
%   yn: posición del eje neutro de la iteración.
...
%    E: matriz de módulos de elasticidad secantes.
%
...
for IELE=1:NELE
    EPZZ(IELE,IPAS)=k*(yn-ECA(IELE,2)); %Deformación longitudinal de la fibra
    [STZZ(IELE,IPAS),ESTA(IELE,IPAS)]=MCM(EPZZ(IELE,IPAS),CAT(ELE(IELE,1),1),MAT); %Esfuerzos en MPa
    %Cálculo del módulo de elasticidad secante
    if IPAS>1
        E(IELE,IPAS)=(STZZ(IELE,IPAS)-STZZ(IELE,IPAS-1))/(EPZZ(IELE,IPAS)-EPZZ(IELE,IPAS-1)); %En MPa
    else
    ...
    G=E(IELE,IPAS)/(2*(1+v));  %Módulo de elasticidad a cortante, en MPa
    EA=EA+E(IELE,IPAS)*ECA(IELE,3);                    %Término asociado a la matriz de rigidez (en desuso)
    EIX=EIX+E(IELE,IPAS)*ECA(IELE,3)*ECA(IELE,2)^2;    %Término asociado a la matriz de rigidez (en desuso)
    EIY=EIY+E(IELE,IPAS)*ECA(IELE,3)*ECA(IELE,1)^2;    %Término asociado a la matriz de rigidez (en desuso)
    GJ=GJ+G*ECA(IELE,3)*(ECA(IELE,1)^2+ECA(IELE,2)^2); %Término asociado a la matriz de rigidez (en desuso)
    FZ=STZZ(IELE,IPAS)*ECA(IELE,3);             %Diferencial de fuerza en la fibra
    MX=STZZ(IELE,IPAS)*ECA(IELE,3)*ECA(IELE,2); %Diferencial de momento
